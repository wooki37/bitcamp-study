...
public class BoardController {
    ...
    public String add(HttpServletRequest request, HttpServletResponse response)
        ...
        if (loginUser == null) {
            request.getParts(); // 일단 클라이언트가 보낸 파일을 읽는다. -> 응답 가능 (없어도 redirect 되긴 됌)
        ...
        try {
            ...
            for (Part part : request.getParts()) {
                if (part.getName().equals("files") && part.getSize() > 0) {
                    String uploadFileUrl = ncpObjectStorageService.uploadFile(
                            "bitcamp-nc7-bucket-12", "board/", part);
            ...
            request.setAttribute("refresh",
                    "2;url=list?category=" + request.getParameter("category"));
            throw e;
        }
    }
    ...
    public String update(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.setAttribute("refresh",
                    "2;url=detail?no=" + request.getParameter("no"));
            throw e;
        }
    }
